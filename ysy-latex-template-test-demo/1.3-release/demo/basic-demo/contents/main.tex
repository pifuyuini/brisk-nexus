...
\section{YsyClass样式特色}

\subsection{颜色盒子}

\begin{uclaim}
	这是盒子uclaim的内容，文字、公式、图片都可以放在这里。
	
	Wishing your day stays bright — like sunshine that chose to stay just for you — is more than just a kind thought. It’s a quiet hope that the world bends a little in your favor today. Imagine the morning sun pausing in its arc across the sky, lingering not out of duty, but out of devotion. Its golden light drapes gently across your shoulders, not with heat, but with warmth — a presence, a reminder that joy doesn’t always shout. Sometimes, it simply shines.
	
	Life moves quickly, often tangled in shadows and shifting moods, but today — just today — let the light settle in. Let it follow you from window to window, from task to task, not as a spotlight but as a companion. Let it spill into the cracks of doubt, soften the edges of worry, and warm the corners where dreams wait patiently.
	
	And if clouds should gather, as they sometimes do, may the memory of that steady sunshine carry you — a whisper of light in your pocket, proof that brightness once chose you, and could again.
\end{uclaim}

\begin{narrator}{讲述者，narrator}
	这是盒子narrator的内容，文字、公式、图片都可以放在这里。
...
\begin{ubox2}{通用2，ubox2}
	这是盒子ubox2的内容，文字、公式、图片都可以放在这里。
...
以下展示了无序列表、颜色文字；并推荐了几种我喜欢的配色主题。

\begin{itemize}
	\item 这是高亮：\highlight{Wishing your day stays bright —— like sunshine that chose to stay just for you.}
	\item 这是强调：\YsyEmph{Wishing your day stays bright —— like sunshine that chose to stay just for you.}
	\item 这是标记：\YsyMark{Wishing your day stays bright —— like sunshine that chose to stay just for you.}
	\item 这是注记：\YsyAnnotation{Wishing your day stays bright —— like sunshine that chose to stay just for you.}
	\item 推荐颜色：
	\begin{itemize}
		\item 漫光天空，SkyRelax；
		\item 优雅的，Elegant；
...
\begin{enumerate}
	\item 微分符号用正体，区别$dx$和$\Diff x$；
	\item 常数用正体，区别$e$，$i$和$\EulerE$，$\ImgI$；
	\item 导数：
	$$
	y'(x) = \Der{y}{x}
	$$
	\item 角标用正体，区别$Y_{MCMC}$和$\DV{Y}{MCMC}$；
	\item 矩阵和向量$\LA{Ax}=\LA{b}$；
\end{enumerate}

\begin{ubox}{规范排版的欧拉公式：}
	\begin{equation}
		\EulerE^{\ImgI\theta} = \cos\theta + \ImgI\sin\theta
	\end{equation}
\end{ubox}

\section{关于SI包siunitx}
\begin{itemize}
	\item 写数字：
	\begin{itemize}
		\item 千分位：\num{123456.789}；
	...
	\item 写单位：\si{\kilogram\metre\per\second}和\si{\joule\per\mole\per\kelvin}；
	
	\item SI指令：\SI{9.8}{\metre\per\second\squared}和\SI{1.602e-19}{\coulomb}；
	
	\item 公式中：
	\begin{itemize}
		\item 不确定度：
		\[
		R = \SI{8.314 \pm 0.001}{\joule\per\mole\per\kelvin}
		\]
		\item 完整公式：
		\[
		E = \frac{1}{2} m v^2 = \SI{0.5}{\kilogram} \cdot \left( \SI{10}{\metre\per\second} \right)^2 = \SI{50}{\joule}
		\]
	\end{itemize}
\end{itemize}
...
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
	...
	\draw [shift={(488,34)}, rotate = 135] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
	% Connection
	\draw    (465,71) -- (526,71) ;
	...
	\draw [shift={(570.6,34)}, rotate = 128.66] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
	% Connection
	\draw    (631.4,58) -- (649.35,35.56) ;
	\draw [shift={(650.6,34)}, rotate = 128.66] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
	% Connection
	\draw    (451,84) -- (451,156) ;
	...
	\draw [shift={(652.33,124)}, rotate = 123.69] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
	% Connection
	\draw    (549.67,158) -- (571.22,125.66) ;
	\draw [shift={(572.33,124)}, rotate = 123.69] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
	% Connection
	\draw    (461.83,158) -- (488.89,125.54) ;
	\draw [shift={(490.17,124)}, rotate = 129.81] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
	% Connection
